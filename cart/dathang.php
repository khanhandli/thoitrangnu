                		 ...
                		 <?php 
                            ...
                            $firstIndex = ($page - 1) * $limit;
                                    if (isset($_GET['timkiem']) && $_GET['timkiem'] != '') {
                                        $sql = "SELECT * FROM GioHang WHERE gia LIKE '%".$_GET['timkiem']."%'";    
                                    }
                                     else {
                                        $sql = 'SELECT * FROM GioHang WHERE 1 LIMIT '.$firstIndex.','.$limit;
                                    }
                                        $classList1 = executeResult($sql);
                                        $sql = 'SELECT count(id) as total FROM GioHang';
...
">';
                ...
                echo    '<button  class= "btn" onclick=\'window.open("detail.php?id='.$class1['id'].'")\'>
                <span class="btn1 btn-edit">Thêm vào giỏ hàng</span></button>';   
                echo				'</div>';
				echo			'</div>';
