<?php 
    ...
    if (!empty($_POST)) {
        ...
        if ($id1 != '') {
            //update 
            $sql = "UPDATE PhuKien SET  Anh ='$anh',  Gia = '$gia' WHERE id = " .$id1;
        }else if($gia != ''){ 
            //insert
            $sql = "INSERT INTO PhuKien(Anh,Gia)
                VALUES('$anh', '$gia')";
                		 ...
                		 <?php 
                            ...
                            $firstIndex = ($page - 1) * $limit;
                                    if (isset($_GET['timkiem']) && $_GET['timkiem'] != '') {
                                        $sql = "SELECT * FROM PhuKien WHERE gia LIKE '%".$_GET['timkiem']."%'";    
                                    }
                                     else {
                                        $sql = 'SELECT * FROM PhuKien WHERE 1 LIMIT '.$firstIndex.','.$limit;
                                    }
                                        $classList1 = executeResult($sql);
                                        $sql = 'SELECT count(id) as total FROM PhuKien';
